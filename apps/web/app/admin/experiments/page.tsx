...
export default async function ExperimentsPage() {
  ...
  return (
    <DashboardPage
      title="Experiments"
      intro="Positioning tests, assignment health, and activation outcomes."
    >
        ...
        {experiments.map((experiment) => (
          <Card key={experiment.id}>
            ...
            <CardContent>
              ...
              <p className="mt-3 font-mono text-[11px] text-muted-foreground">{allocation(experiment.allocation)}</p>
              <Link
                href={`/admin/experiments/${experiment.id}`}
                className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline"
              >
                Open experiment <ArrowUpRight className="size-3.5" />
              </Link>
            </CardContent>
          </Card>
        ))}
